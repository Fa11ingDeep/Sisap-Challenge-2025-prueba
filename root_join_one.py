    ...
    for h in range((maxSize - 1) * len(centers)):
        ...
        while True:
            ...
            if len(groups[group_id1][0]) < maxSize:  # If the group size is less than the maximum size.
                ...
                for l in range(1):  # Get the centers and add the first two centers to the list.
        ...
        while o < len(data):
            ...
            while True:
                ...
                if len(groups[group_id1][0]) < extended_size_group:  # If the group size is less than the maximum size.
                    ...
                    next_group_list = []  # List of centers.
                    for u in range(1):  # Get the centers and add the first two centers to the list.
                    ...
                    if(dist1<groups[group_id1][1]): # If the distance id less than the radius, insert the new point anyway.
                        ...
                        next_group_list=[] # List of centers.
                        for p in range(1): # Get the centers and add the first two centers to the list.
...
def run(dataset, task, k):
    ...
    try:
        ...
        store_results(os.path.join("results/", dataset, task, f"root_join.h5"), 'Root_Join', 'gooaq', 'task2', D, I, d_dim_red, total_global, f'root_join_params: 1,1; PCA params: 0.8; one')
